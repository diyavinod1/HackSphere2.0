...
#     try:
#         # Load the webm file with librosa (it can handle webm with ffmpeg backend)
...
#             print("Audio too short, using synthetic features")
#             return generate_synthetic_features("short")
...
#         # Fundamental frequency (pitch) related features
...
#             if len(f0_valid) > 0:
#                 features['meanF0'] = np.mean(f0_valid)
#                 features['stdF0'] = np.std(f0_valid)
#                 features['minF0'] = np.min(f0_valid)
#                 features['maxF0'] = np.max(f0_valid)
#             else:
#                 features['meanF0'] = 180
#                 features['stdF0'] = 8
#                 features['minF0'] = 150
...
#         if 'meanF0' in features and features['meanF0'] > 0:
#             jitter_abs = features['stdF0'] * 0.1  # Approximation
#             jitter_rel = jitter_abs / features['meanF0']
#         else:
#             jitter_abs = 0.005
#             jitter_rel = 0.02
        
#         features['jitter_abs'] = jitter_abs
#         features['jitter_rel'] = jitter_rel
...
#             if len(rms) > 1:
#                 shimmer_abs = np.std(rms) * 0.5  # Approximation
#                 shimmer_rel = shimmer_abs / np.mean(rms) if np.mean(rms) > 0 else 0.05
#             else:
#                 shimmer_abs = 0.03
#                 shimmer_rel = 0.05
#         except:
...
#         except:
#             for i in range(13):
#                 features[f'mfcc_{i+1}_mean'] = np.random.normal(0, 10)
#                 features[f'mfcc_{i+1}_std'] = np.random.normal(5, 2)
...
#             features['spectral_centroid_std'] = 300
#             features['spectral_rolloff_mean'] = 4000
#             features['spectral_rolloff_std'] = 500
        
#         # Add spectral contrast features
#         for i in range(7):
#             features[f'spectral_contrast_{i+1}_mean'] = np.random.normal(10, 2)
#             features[f'spectral_contrast_{i+1}_std'] = np.random.normal(3, 0.5)
...
#         import traceback
#         traceback.print_exc()
#         return generate_synthetic_features("error")

# def generate_synthetic_features(reason="default"):
#     """Generate synthetic features for testing"""
#     print(f"Generating synthetic features due to: {reason}")
...
#         features.update({
#             'meanF0': 180, 'stdF0': 5, 'minF0': 150, 'maxF0': 210,
#             'jitter_abs': 0.002, 'jitter_rel': 0.005,
#             'shimmer_abs': 0.02, 'shimmer_rel': 0.03,
#             'HNR': 20
#         })
#     elif reason == "parkinsons":
#         # Parkinson's voice features
#         features.update({
#             'meanF0': 150, 'stdF0': 15, 'minF0': 100, 'maxF0': 220,
#             'jitter_abs': 0.01, 'jitter_rel': 0.02,
#             'shimmer_abs': 0.08, 'shimmer_rel': 0.1,
#             'HNR': 12
#         })
#     else:
#         # Random features
#         features.update({
#             'meanF0': np.random.normal(165, 20),
#             'stdF0': np.random.normal(10, 5),
#             'minF0': np.random.normal(130, 20),
#             'maxF0': np.random.normal(200, 25),
#             'jitter_abs': np.random.normal(0.005, 0.003),
#             'jitter_rel': np.random.normal(0.01, 0.005),
#             'shimmer_abs': np.random.normal(0.04, 0.02),
#             'shimmer_rel': np.random.normal(0.06, 0.03),
#             'HNR': np.random.normal(16, 4)
#         })
    
#     # Add MFCC features
#     for i in range(13):
#         features[f'mfcc_{i+1}_mean'] = np.random.normal(0, 12)
#         features[f'mfcc_{i+1}_std'] = np.random.normal(6, 2)
    
#     # Add spectral features
#     features.update({
#         'spectral_centroid_mean': np.random.normal(1900, 500),
#         'spectral_centroid_std': np.random.normal(400, 100),
#         'spectral_rolloff_mean': np.random.normal(3800, 800),
#         'spectral_rolloff_std': np.random.normal(600, 150)
#     })
    
#     # Add spectral contrast features
#     for i in range(7):
#         features[f'spectral_contrast_{i+1}_mean'] = np.random.normal(9, 3)
#         features[f'spectral_contrast_{i+1}_std'] = np.random.normal(4, 1)
...
#     healthy_data = []
#     for _ in range(100):
#         features = generate_synthetic_features("healthy")
#         healthy_data.append(features)
    
#     # Parkinson's patients (class 1)
#     parkinsons_data = []
#     for _ in range(100):
#         features = generate_synthetic_features("parkinsons")
#         parkinsons_data.append(features)
    
#     # Create DataFrames
#     healthy_df = pd.DataFrame(healthy_data)
#     healthy_df['status'] = 0
    
#     parkinsons_df = pd.DataFrame(parkinsons_data)
#     parkinsons_df['status'] = 1
    
#     # Combine datasets
#     df = pd.concat([healthy_df, parkinsons_df], ignore_index=True)
...
#         return True
#     except Exception as e:
#         print(f"No saved model found: {e}, training new model")
...
#         print(f"Audio saved to {webm_path}")
        
#         # Convert webm to wav
#         if not convert_webm_to_wav(webm_path, wav_path):
#             # If conversion fails, use synthetic features
#             features = generate_synthetic_features("conversion_failed")
#         else:
#             # Extract features from converted wav
#             features = extract_features(wav_path)
        
#         if features is None:
#             features = generate_synthetic_features("extraction_failed")
...
#         if prediction == 0:
#             result = "Healthy"
#             interpretation = "Your voice analysis shows characteristics typical of healthy individuals."
#             color = "green"
#         else:
#             result = "At Risk"
#             interpretation = "Your voice analysis shows some characteristics that may be associated with Parkinson's disease. Please consult a healthcare professional for further evaluation."
#             color = "orange"
...
#         print(f"Analysis complete: {result}, Risk score: {risk_score}%")
...
#         # Return a demo result if everything fails
#         return jsonify({
#             'result': "Healthy",
#             'risk_score': 15,
#             'interpretation': "Demo result: Your voice analysis shows characteristics typical of healthy individuals.",
#             'color': "green",
#             'plot_url': generate_demo_plot()
#         })

# def generate_feature_plot(features, result):
#     try:
#         # Select key features for visualization
#         key_features = ['meanF0', 'stdF0', 'jitter_rel', 'shimmer_rel', 'HNR']
#         feature_names = ['Mean Pitch', 'Pitch Variability', 'Jitter', 'Shimmer', 'HNR']
...
#         ax.set_title('Voice Feature Analysis')
...
#         return generate_demo_plot()

# def generate_demo_plot():
#     """Generate a demo plot when real plotting fails"""
#     try:
#         fig, ax = plt.subplots(figsize=(10, 6))
#         features = ['Mean Pitch', 'Pitch Variability', 'Jitter', 'Shimmer', 'HNR']
#         values = [180, 8, 0.015, 0.045, 18]
        
#         bars = ax.bar(features, values, color='#4CAF50', alpha=0.7)
...
#         ax.set_ylabel('Feature Value')
#         ax.set_title('Voice Feature Analysis (Demo)')
#         ax.grid(axis='y', alpha=0.3)
#         plt.xticks(rotation=45)
#         plt.tight_layout()
...
# @app.route('/demo')
# def demo():
#     """Demo endpoint that always returns a successful result"""
...
#     result = "Healthy" if prediction == 0 else "At Risk"
#     interpretation = "This is a demo result showing how the analysis works."
...
#     print("Starting Parkinson's Voice Detection App...")
#     load_model()
#     print("App is ready! Visit http://localhost:5000")
